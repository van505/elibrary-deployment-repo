...
@section('content')
    ...
    <div class="h-full"
        x-data="Object.assign(categoryDrawer(), { viewMode: localStorage.getItem('categoriesView') || 'grid' })"
        ...
        <div class="space-y-5">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-2">
                <div
                    class="bg-white rounded-xl border border-gray-100 border-t-2 border-t-amber-500 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.07),0_2px_4px_-2px_rgba(0,0,0,0.05)] p-5 flex items-center gap-4 transition-all duration-200 hover:shadow-[0_10px_15px_-3px_rgba(0,0,0,0.08),0_4px_6px_-4px_rgba(0,0,0,0.05)] hover:-translate-y-0.5">
                    <div
                        class="w-11 h-11 bg-amber-50 rounded-xl flex items-center justify-center flex-shrink-0 ring-1 ring-amber-100 shadow-sm">
                ...
                <div
                    class="bg-white rounded-xl border border-gray-100 border-t-2 border-t-amber-500 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.07),0_2px_4px_-2px_rgba(0,0,0,0.05)] p-5 flex items-center gap-4 transition-all duration-200 hover:shadow-[0_10px_15px_-3px_rgba(0,0,0,0.08),0_4px_6px_-4px_rgba(0,0,0,0.05)] hover:-translate-y-0.5">
                    <div
                        class="w-11 h-11 bg-emerald-50 rounded-xl flex items-center justify-center flex-shrink-0 ring-1 ring-emerald-100 shadow-sm">
                ...
                <div
                    ...
                    <div>
                        <div class="text-base font-semibold text-gray-900 truncate max-w-[100px]">
                            {{ $mostUsed?->name ?? '—' }}
                        </div>
                ...
                <div
                    ...
                    <div>
                        <div class="text-2xl font-bold text-gray-900">{{ $ebooksCategorized }}</div>
                        <div class="text-sm text-gray-500">Ebooks Categorized</div>
                    </div>
                </div>
            </div>

            {{-- ── Filter Bar ──────────────────────────────────────────── --}}
            <x-admin.filter-bar :action="route('admin.categories.index')" searchPlaceholder="Search category name..."
                :sortable="['created_at' => 'Date Added', 'name' => 'Name']">
            </x-admin.filter-bar>

            {{-- ── Categories Grid/Table ────────────────────────────────── --}}
            {{-- Grid View --}}
            <div x-show="viewMode === 'grid'" style="display:none;" x-data="{ show: false }"
                    ...
                    @forelse($categories as $index => $category)
                        ...
                        <div x-show="show" x-transition:enter="transition ease-out duration-300"
                            ...
                            <div class="h-20 flex items-center justify-center"
                                style="background: linear-gradient(135deg, {{ $catHex }}33, {{ $catHex }}66);">
                                <span class="text-3xl font-black"
                                    style="color: {{ $catHex }}">{{ strtoupper(substr($category->name, 0, 1)) }}</span>
                            </div>
                            {{-- Body --}}
                            <div class="p-4 relative">
                                ...
                                <div class="pr-6">
                                    ...
                                    <p class="text-xs text-gray-500 mt-0.5">{{ $category->ebooks_count ?? 0 }}
                                        ebook{{ ($category->ebooks_count ?? 0) !== 1 ? 's' : '' }}</p>
            ...
            <div x-show="viewMode === 'table'" style="display:none;"
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($categories as $category)
                            ...
                            <tr class="hover:bg-gray-50/60 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <code
                                        class="font-mono text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{{ $category->slug }}</code>
                                </td>
                                {{-- Book Count --}}
                                <td class="px-6 py-4">
                                    <span
                                        class="text-xs font-medium text-gray-500">{{ $category->ebooks_count ?? $category->ebooks()->count() }}
                                        books</span>
                                </td>
                                {{-- Description --}}
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2 justify-end">
                                        {{-- Edit pencil --}}
                                        <button type="button" @click="openEdit({{ $category->id }}, {{ Js::from($catInline) }})"
                ...
                <x-admin.drawer title="Add Category" edit-title="Edit Category"
                        ...
                        <div class="col-span-1">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach($colorMap as $colorName => $colorVal)
                                    <button type="button" @click="formData.color = '{{ $colorName }}'"
                                        ...
                                        :class="formData.color === '{{ $colorName }}'
                                                ...
                                                : 'hover:scale-110'" style="background-color: {{ $colorVal['hex'] }}">
                                        <svg x-show="formData.color === '{{ $colorName }}'" class="w-3 h-3 text-white" fill="none"
